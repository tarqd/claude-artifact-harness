...
declare namespace Claude {
  namespace room {
     ...
     *  -- send when ready"), not a receipt of what Claude did with it:
     *  "pane" -- the conversation open beside this page took it; "session" --
     *  a Claude session of this viewer's that has the artifact open;
     *  "new" -- a conversation was started for it. Treat an unrecognized
     *  value as "pane". */
    interface SentTo {
      to: "pane" | "session" | "new";
    }
     ...
     *  outside one is refused. It STAGES: `data` appears in the viewer's
     *  composer as one attachment they can see and remove, named for this
     *  artifact (with your `label`, when given), a newer send from this page
     *  replaces it, and nothing reaches Claude until the viewer sends their
     *  own message -- there is no way for a page to start Claude's turn. To
     *  write a comment and bring Claude into its thread, use the `comments`
     *  capability's `sendToClaude` instead. Resolves `SentTo` once the
     *  platform has taken it. Rejects, never throws: `invalid_argument`
     ...
     *  the code. */
    function sendToClaudeSession(data: ToClaude): Promise<SentTo>;
     ...
     *  on "available"; on any other value or any rejection (a platform that
