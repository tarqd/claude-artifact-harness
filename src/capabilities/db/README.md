...
Not implemented. `frame.ts` mounts nothing (so `claude.use("db")`
resolves `null`), `broker.ts` rejects every call with
`capability_disabled`, and `server.ts` registers no routes.

The db slice replaces these three files and this README. Nothing outside
`src/capabilities/db/`, `test/db/` and `fixtures/db*.html`
