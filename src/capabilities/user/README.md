...
Not implemented. `frame.ts` mounts nothing (so `claude.use("user")`
resolves `null`), `broker.ts` rejects every call with
`capability_disabled`, and `server.ts` registers no routes.

The user slice replaces these three files and this README. Nothing outside
`src/capabilities/user/`, `test/user/` and `fixtures/user*.html`
