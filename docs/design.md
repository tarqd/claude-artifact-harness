...
- Host and origin guards (`src/server/guards.ts`), mounted on both apps ahead of every route: the `Host` must be `SHELL_HOST` on the shell and `<artifactId>.<FRAME_HOST_SUFFIX>` (or the bare suffix, for the `/_a/` form) on the frame origin, plus loopback literals and `ARTIFACT_ALLOWED_HOSTS` — a rebound name is not a same-origin client of the shell just because it resolves to the loopback bind. Non-GET requests on the shell origin must additionally carry `Sec-Fetch-Site: same-origin|none` and, if any, an `Origin` equal to this origin, which is what refuses an artifact frame's `credentials: "include"` write under a shared registrable domain; a client sending neither header must post `application/json`, the one exemption being the `assets` upload lane, where the blob's own media type is taken and only the CORS-safelisted encodings (and a missing type) are refused.
